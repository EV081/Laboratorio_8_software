...
El flujo principal arranca con Registrar Cena (UC1), que el restaurante invoca a través de la API REST. Este caso siempre incluye Publicar Evento de Transacción(UC2), porque el registro no termina hasta que el evento se publica en RabbitMQ. UC2 a su vez incluye  Calcular Puntos / Cashback (UC3), que aplica la política configurada (por defecto, 10 % del consumo), y este se incluye dentro de Actualizar Cuenta de Recompensas (UC4), que persiste los puntos en la cuenta del cliente.
